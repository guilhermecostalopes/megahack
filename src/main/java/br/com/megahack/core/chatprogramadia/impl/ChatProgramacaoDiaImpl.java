...
public class ChatProgramacaoDiaImpl implements ChatProgramacaoDiaService {
	...
	private void alterarResource(ChatProgramacaoDiaResource resource, ChatProgramacaoDia entidade) {
		...
		resource.setHoraInicio(
				StringUtils.leftPad(hora.toString(), 2, "0") + "/" + StringUtils.leftPad(minuto.toString(), 2, "0")
						+ "/" + StringUtils.leftPad(segundo.toString(), 2, "0"));
		resource.setDiaSemana(entidade.getProgramaDia().getDiaSemana().getDescricao());
		resource.setPrograma(entidade.getProgramaDia().getProgramacao().getNome());
		resource.setRegiao(entidade.getProgramaDia().getRegiao().getNome());
